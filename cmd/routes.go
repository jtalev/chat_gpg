...
func add_routes(mux *http.ServeMux, ctx context.Context, h *handlers.Handler, a *infrastructure.Auth) {
	...
	mux.Handle("/leave/leave-request-modal", a.AuthMiddleware(h.EmployeeLeaveModal()))
	...
	mux.Handle("/employee/post", a.AuthMiddleware(h.PostEmployee()))
	...
	mux.Handle("/reports/prev-timesheet-report", a.AuthMiddleware(h.PrevEmployeeTimesheetReport()))
	mux.Handle("/reports/next-timesheet-report", a.AuthMiddleware(h.NextEmployeeTimesheetReport()))
	...
	mux.Handle("/admin/add-employee-modal", a.AuthMiddleware(h.AddEmployeeModal()))
}
